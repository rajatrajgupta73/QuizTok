...
def render() -> None:
    ...
    top_l.markdown(f'<div class="qt-h1" style="font-size:34px">Command Center 🎛️</div>'
...
def _dashboard() -> None:
    ...
    ui.kpi_card(c1, str(k["games_hosted"]), "GAMES HOSTED")
    ui.kpi_card(c2, str(k["total_players"]), "HUMAN PLAYERS")
    ui.kpi_card(c3, k["avg_accuracy"], "AVG ACCURACY")
    ui.kpi_card(c4, f"{len(bank):,}", "QUESTIONS IN BANK")
...
def _quiz_library(email: str) -> None:
    ...
    for _, row in quizzes.iloc[::-1].iterrows():
        n_q = storage.question_count(str(row["quiz_id"]))
        col1, col2 = st.columns([4, 1])
        col1.markdown(
            f'<div class="qt-card" style="padding:14px 20px">'
            f'<span style="font-size:24px">{row["emoji"]}</span> <b>{row["title"]}</b>'
            f'<span class="qt-sub" style="margin-left:10px">{n_q} Qs · {row["category"]} · '
            f'by {row["created_by"]}</span></div>', unsafe_allow_html=True)
